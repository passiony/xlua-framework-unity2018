...
namespace AssetBundles
{
    [Hotfix]
    [LuaCallCSharp]
    public class WebAssetRequester : BaseAssetRequester
    {
        static Queue<WebAssetRequester> pool = new Queue<WebAssetRequester>();
        static int sequence = 0;
        protected WWW www = null;
        protected bool isOver = false;

        public static WebAssetRequester Get()
        {
            if (pool.Count > 0)
            {
                return pool.Dequeue();
            ...
            {
                return new WebAssetRequester(++sequence);
            }
        }

        public static void Recycle(WebAssetRequester creater)
        {
            pool.Enqueue(creater);
        }

        public WebAssetRequester(int sequence)
        {
            Sequence = sequence;
        }
